...
Ready_: while(command!=MISS_START){
	...
	//########################################################################################//
	cout<<"Mission shall commence within 7 minutes"<<endl;
	sprintf(gen_msg_buffer,"MH:Mission shall commence within 2 minutes!!!\n");
	log_missionstatus(sys_filename,gen_msg_buffer);
	_sleep(2*60*1000);
...
Start_:		if(feof(fp_missfile))
			...
			if(depth_err_computed){
				if(abs(depth_err)<=0.1){
					sprintf(gen_msg_buffer,"MH:Depth Stabilization ON");
					log_missionstatus(sys_filename,gen_msg_buffer);
					while(count--){
						//send sensor data to surface control as regards current postion and altimetry
						Phins_Daq();
						...
						//cout<<sds.veh_east_pos<<" "<<sds.veh_north_pos<<" "<<sds.latitude<<endl;
						_sleep(10);
						cout<<"stabilizing depth"<<endl;
						//// Log sensor data 
						log_systemdata(mission_filename);
					}
					sprintf(gen_msg_buffer,"MH:Desired Depth Correction Achieved/Stabilized");
					log_missionstatus(sys_filename,gen_msg_buffer);
...
unsigned long _stdcall surge_ctrl(void *data){
	...
	while(1){
		...
		if(rds.sg_des!=0){
			...
			if((cds.P+cds.D)<0){
				if((cds.P+cds.D-1.3)<-4.0)
					cds.signal=-4.0;
				else
					cds.signal=cds.P+cds.D-1.3;
			}
			else{
				if((cds.P+cds.D+1.3)>4.0)
					cds.signal=4.0;
...
void get_batt_status(){
	...
	{
		cout<<"ADAMTCP_SendReceive5KTCPCmd() Fail !!!"<<endl;
	}
...
void log_systemdata(char *filename){
	...
	fprintf(fp_log,"%s %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n",
			time_buffer,
			sds.veh_depth,
			dds.disp_Z,
			sds.veh_alti,
